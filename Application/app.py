...
class ProgressCallback(Callback):
    """Callback Keras untuk mengupdate progress bar di Tkinter."""
...
class App:
    def __init__(self, root):
        ...
        # Bagian Atas: Kontrol dan Tampilan Data
        top_section = tk.Frame(main_container, bg='#f0f0f0')
        top_section.pack(fill=tk.BOTH, expand=True, pady=(0, 5))

        # Panel Kiri: Kontrol
        left_panel = tk.Frame(top_section, width=300, bg='#ffffff', relief=tk.RAISED, bd=2)
        ...
        tk.Label(header_frame, text="Control Panel", bg='#2c3e50', fg='white', font=('Arial', 14, 'bold')).pack(expand=True)

        # Frame untuk File & Proses
        control_frame = tk.Frame(left_panel, bg='#ffffff')
        control_frame.pack(fill=tk.X, padx=10, pady=10)

        self.btn_load = tk.Button(control_frame, text="📁 Pilih File CSV", command=self.load_file, bg='#3498db', fg='white', font=('Arial', 11, 'bold'), relief=tk.FLAT, pady=8)
        self.btn_load.pack(fill=tk.X, pady=(0, 10))

        self.lbl_file = tk.Label(control_frame, text="Belum ada file dipilih", wraplength=280, justify="center", bg='#ffffff', fg='#7f8c8d', font=('Arial', 9))
        self.lbl_file.pack(pady=5)

        self.btn_process = tk.Button(control_frame, text="🚀 Latih Model BiLSTM", command=self.process_data, state=tk.DISABLED, bg='#27ae60', fg='white', font=('Arial', 11, 'bold'), relief=tk.FLAT, pady=8)
        self.btn_process.pack(fill=tk.X, pady=(10, 5))
        
        # Info Dataset
        info_frame = tk.Frame(left_panel, bg='#ecf0f1', relief=tk.SUNKEN, bd=1)
        info_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        tk.Label(info_frame, text="Informasi Dataset", bg='#ecf0f1', font=('Arial', 11, 'bold')).pack(pady=5)
        self.lbl_info = tk.Label(info_frame, text="", bg='#ecf0f1', fg='#2c3e50', font=('Arial', 10), justify='left', anchor='nw')
        self.lbl_info.pack(pady=5, padx=10, fill='both', expand=True)

        # Panel Kanan: Notebook untuk Preview dan Hasil
        ...
        # Bagian Bawah: Visualisasi
        bottom_section = tk.Frame(main_container, height=350, bg='#ffffff', relief=tk.RAISED, bd=2)
        ...
        tk.Label(viz_header, text="Area Visualisasi", bg='#8e44ad', fg='white', font=('Arial', 12, 'bold')).pack(expand=True)
        ...
        tk.Label(btn_frame, text="Pilih Visualisasi:", bg='#f0f0f0', font=('Arial', 10, 'bold')).pack(side=tk.LEFT, padx=(10, 5))
        ...
        for text, command, color in buttons_config:
            btn = tk.Button(btn_frame, text=text, command=command, bg=color, fg='white', font=('Arial', 9, 'bold'), relief=tk.FLAT, padx=12, pady=5)
    ...
    def create_treeview(self, parent):
        ...
        tree = ttk.Treeview(tree_frame, yscrollcommand=tree_scroll_y.set, xscrollcommand=tree_scroll_x.set, selectmode="extended")
    ...
    def load_file(self):
        ...
        try:
            ...
            if 'text' in self.df.columns and 'sentiment' in self.df.columns:
                ...
                messagebox.showwarning("Warning", "Kolom 'text' dan 'sentiment' wajib ada.")
    ...
    def show_results_dataset(self):
        ...
        if 'pred_label' in cols and 'sentiment' in cols:
            cols = ['text', 'sentiment', 'pred_label'] + [c for c in cols if c not in ['text', 'sentiment', 'pred_label']]
    ...
    def process_data(self):
        ...
        progress_label = tk.Label(progress_window, text="Mempersiapkan data...", font=('Arial', 10))
        ...
        try:
            ...
            self.history = self.model.fit(X, y_cat,
                                          epochs=epochs,
                                          batch_size=32,
    ...
    def show_wordcloud(self):
        ...
        stopwords_id = {'dan', 'yang', 'di', 'ke', 'dari', 'untuk', 'pada', 'adalah', 'ini', 'itu',
    ...
    'sampe', 'tiap', 'pas', 'lewat', 'point', 'msh', 'udh', 'maaf', 'lihat', 'yaudah', 'banget', 'terus', 'masih'}
        ...
        wordcloud = WordCloud(width=800, height=400, background_color='white', stopwords=custom_stopwords, collocations=False, max_words=150).generate(text)
    ...
    def show_confusion_matrix(self):
        ...
        sns.heatmap(cm, annot=True, fmt="d", cmap="viridis", xticklabels=labels, yticklabels=labels, annot_kws={"size": 14})
    ...
    def show_classification_report(self):
        ...
        report = classification_report(self.df['sentiment'], self.df['pred_label'], output_dict=True, zero_division=0)
        ...
        tbl = ax.table(cellText=df_report.values, rowLabels=df_report.index, colLabels=df_report.columns, cellLoc='center', loc='center')
    ...
    def show_training_history(self):
        if self.history is None:
            messagebox.showwarning("Warning", "Riwayat training tidak ditemukan. Latih model terlebih dahulu.")
            return
        ...
        # Akurasi
        ax1.plot(self.history.history['accuracy'], 'o-', label='Training Accuracy', color='#2ecc71')
        ax1.plot(self.history.history['val_accuracy'], 'o-', label='Validation Accuracy', color='#e74c3c')
        ax1.set_title('Model Accuracy', fontsize=14); ax1.set_xlabel('Epoch'); ax1.set_ylabel('Accuracy')
        ax1.legend(); ax1.grid(True, linestyle='--', alpha=0.6)

        # Loss
        ax2.plot(self.history.history['loss'], 'o-', label='Training Loss', color='#3498db')
        ax2.plot(self.history.history['val_loss'], 'o-', label='Validation Loss', color='#f39c12')
        ax2.set_title('Model Loss', fontsize=14); ax2.set_xlabel('Epoch'); ax2.set_ylabel('Loss')
